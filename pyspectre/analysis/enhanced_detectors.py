...
class EnhancedDivisionByZeroDetector(EnhancedDetector):
    ...
    def check(self, ctx: DetectionContext) -> Issue | None:
        ...
        if divisor_var and self._is_guarded_by_zero_check(ctx, divisor_var):
            return None
        if ctx.can_pattern_suppress("ZeroDivisionError"):
        ...
        if divisor_var and self._has_prior_assertion(ctx, divisor_var):
            return None
        op_name = "/" if "/" in instr.argrepr else "%"
        if divisor_var:
...
class EnhancedKeyErrorDetector(EnhancedDetector):
    ...
    def check(self, ctx: DetectionContext) -> Issue | None:
        ...
        if ctx.is_in_try_block("KeyError"):
            ...
            return self.suppress_issue(issue, "Caught by exception handler")
        if self._has_key_check(ctx, container_var, key_var):
            return None
    ...
    def _key_known_to_exist(
        ...
        if container_type.element_types and key_value is not None:
...
class EnhancedIndexErrorDetector(EnhancedDetector):
    ...
    def check(self, ctx: DetectionContext) -> Issue | None:
        ...
        if index_value is not None and container_type.length is not None:
            length = container_type.length
            if isinstance(index_value, int):
                if index_value >= 0 and index_value >= length:
                    return self.create_issue(
                        ctx,
                        message=f"IndexError: index {index_value} out of range for `{container_var}` of length {length}",
                        confidence=0.95,
                    )
                elif index_value < 0 and abs(index_value) > length:
                    return self.create_issue(
                        ctx,
                        message=f"IndexError: negative index {index_value} out of range for `{container_var}` of length {length}",
                        confidence=0.95,
                    )
                else:
                    return None
        if self._has_bounds_check(ctx, container_var, index_var):
            return None
...
class EnhancedAnalyzer:
    ...
    def analyze_function(
        ...
        if not instructions:
            return issues
        type_env = self.type_analyzer.analyze_function(code)
        pattern_info = self.pattern_analyzer.analyze_function(code, type_env)
        ...
        for instr in instructions:
            ...
            flow_context = None
            if flow_analyzer:
                flow_context = FlowContext.create(flow_analyzer, instr.offset)
            ctx = DetectionContext(
                code=code,
